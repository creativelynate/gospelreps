export const movements = [
  {
    id: '2-5-1',
    name: '2–5–1',
    description: 'The backbone of gospel resolution.',
    tags: ['beginner', 'worship'],
    defaultKey: 'C',
    chords: [
      { name: '2min7', degree: 2, type: 'min7', inversions: [0, 1, 2] },
      { name: '5dom7', degree: 5, type: 'dom7', inversions: [0, 1, 2] },
      { name: '1maj7', degree: 1, type: 'maj7', inversions: [0, 1, 2, 3] },
    ],
  },
  {
    id: '1-4-5-1',
    name: '1–4–5–1',
    description: 'Classic church cadence. Foundation of praise & worship.',
    tags: ['beginner', 'worship'],
    defaultKey: 'C',
    chords: [
      { name: '1', degree: 1, type: 'maj', leftHand: { degree: 1 }, inversions: [0, 1, 2] },
      { name: '4', degree: 4, type: 'maj', leftHand: { degree: 4 }, inversions: [0, 1, 2] },
      { name: '5', degree: 5, type: 'maj', leftHand: { degree: 5 }, inversions: [0, 1, 2] },
      { name: '1', degree: 1, type: 'maj', leftHand: { degree: 1 }, inversions: [0, 1, 2] },
    ],
  },
  {
    id: 'passing-dim',
    name: 'Passing Dim',
    description: 'Chromatic tension into resolution. Essential shout music tool.',
    tags: ['advanced', 'shout'],
    defaultKey: 'C',
    chords: [
      { name: '1',     degree: 1, type: 'maj' },
      { name: '#1dim', degree: 1, type: 'dim', chromaticRoot: 1 },
      { name: '2min7', degree: 2, type: 'min7', inversions: [0, 1, 2] },
    ],
  },
  {
    id: '1-6-2-5',
    name: '1–6–2–5',
    description: 'The gospel turnaround. Keeps the energy cycling.',
    tags: ['advanced', 'worship'],
    defaultKey: 'C',
    chords: [
      { name: '1maj7', degree: 1, type: 'maj7', inversions: [0, 1, 2, 3] },
      { name: '6min7', degree: 6, type: 'min7', inversions: [0, 1, 2, 3] },
      { name: '2min7', degree: 2, type: 'min7', inversions: [0, 1, 2, 3] },
      { name: '5dom7', degree: 5, type: 'dom7', inversions: [0, 1, 2, 3] },
    ],
  },
  {
    id: 'shout-vamp',
    name: 'Shout Vamp',
    description: 'The groove under the shout break. Rhythm + repetition.',
    tags: ['shout', 'advanced'],
    defaultKey: 'C',
    chords: [
      { name: '4', degree: 4, type: 'maj', leftHand: { degree: 4 }, inversions: [0, 1, 2] },
      { name: '5', degree: 5, type: 'maj', leftHand: { degree: 5 }, inversions: [0, 1, 2] },
      { name: '1', degree: 1, type: 'maj', leftHand: { degree: 1 }, inversions: [0, 1, 2] },
    ],
  },
  {
    id: 'passing-dim-minor',
    name: 'Passing Dim Minor',
    description: 'Great way to transition into the minor 6.',
    tags: ['advanced', 'shout'],
    defaultKey: 'C',
    chords: [
      { name: '1',     degree: 1, type: 'maj' },
      { name: '#5dim', degree: 5, type: 'dim', chromaticRoot: 1 },
      { name: '6min',  degree: 6, type: 'min', inversions: [0, 1, 2] },
    ],
  },
  {
    id: 'passing-dim-minor2',
    name: '5 Passing to Minor 6',
    description: 'Great way to transition into the minor 6.',
    tags: ['advanced', 'shout'],
    defaultKey: 'C',
    chords: [
      { name: '5',     degree: 5, leftHand: { degree: 5 }, type: 'maj', octaveShift: -1 },
      { name: '5',     degree: 5, leftHand: { degree: 7 }, type: 'maj', octaveShift: -1 },
      { name: '1aug',  degree: 1, leftHand: { degree: 3 }, type: 'aug', octaveShift: -1, inversions: [0, 1, 2] },
      { name: '#5dim', degree: 5, leftHand: { degree: 5, chromaticOffset: 1 }, type: 'dim', octaveShift: -1, chromaticRoot: 1 },
      { name: '6min',  degree: 6, leftHand: { degree: 6 }, type: 'min', octaveShift: -1, inversions: [0, 1, 2] },
    ],
  },
]

export const ALL_KEYS = ['C', 'Db', 'D', 'Eb', 'E', 'F', 'F#', 'G', 'Ab', 'A', 'Bb', 'B']